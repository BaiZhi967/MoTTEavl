...
数据来自本地 JSONL（项目内也内置若干样例），因此没有官方上下游；契约保证的是：
...
匹配才算本套件记录；不匹配的记录一律当普通资源处理，不享受不可变与严格校验。
"""
...
def _preset(case_count: int, scorer: str) -> dict[str, Any]:
    return {
        "suite": SUITE, "id": DATASET_ID, "version": DATASET_VERSION,
        "selected_count": case_count, "selection": SELECTION_ALL, "scorer": scorer,
...
def _input_of(raw: dict[str, Any], line_no: int) -> str:
    ...
    if len(present) != 1 or not isinstance(present[0], str) or not present[0].strip():
        raise ValueError(f"invalid input/prompt at source line {line_no}: exactly one non-empty "
...
def _parse_lines(raw: bytes, name: str) -> list[dict[str, Any]]:
    ...
    for line_no, line in enumerate(text.splitlines(), 1):
        ...
        if expected is not None and (not isinstance(expected, str) or not expected.strip()):
            raise ValueError(f"invalid expected at source line {line_no}: must be a non-empty string")
        rows.append({
            "case_id": _case_id_of(row, name, len(rows)),
            "input": _input_of(row, line_no),
            "expected": expected,
            "scorer": None if row.get("scorer") in (None, "") else normalize_scorer(row["scorer"]),
            "source_line": line_no,
        })
    if not rows:
...
def import_direct_llm_jsonl(
    raw: bytes, *, name: str, version: str, license_id: str, scorer: str | None = None,
    source: str = "local-jsonl", synthetic: bool = False,
) -> dict[str, Any]:
    ...
    if not all(isinstance(value, str) and value.strip()
               for value in (name, version, license_id, source)):
    ...
    for row in rows:
        ...
        case: dict[str, Any] = {"case_id": row["case_id"], "input": row["input"],
                                "metadata": metadata}
    ...
    record = {
        ...
        "provenance": {"source": source, "license": license_id,
...
def _check_cases(cases: Any) -> list[Any]:
    ...
    for case in validated.cases:
        ...
        if (not case.case_id or not isinstance(case.input, str) or not case.input.strip()
                or unknown or type(line) is not int or line < 1
                or (case.expected is not None and (not isinstance(case.expected, str)
...
def validate_dataset(record: dict[str, Any]) -> None:
    ...
    if (type(source_lines) is not int or source_lines < len(cases)
            or type(provenance.get("synthetic")) is not bool
            or not all(isinstance(provenance.get(key), str) and provenance[key].strip()
...
def scenario_for(dataset: dict[str, Any], *, version: str) -> dict[str, Any]:
    ...
    return {"name": dataset["name"], "version": version, "mode": "direct-llm",
            EVAL_KEY: dict(dataset[EVAL_KEY]), "dataset": f"{dataset['name']}@{dataset['version']}"}


def validate_scenario(record: dict[str, Any]) -> None:
    eval_spec = _eval_of(record)
    count = eval_spec.get("selected_count") if eval_spec is not None else None
    if (eval_spec is None or record.get("mode") != "direct-llm"
...
def effective_scorers(dataset: dict[str, Any]) -> dict[str, str]:
    ...
    return {case["case_id"]: normalize_scorer((case.get("metadata") or {}).get("scorer")
                                              or dataset_scorer)
