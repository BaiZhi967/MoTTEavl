"""M4-T11：Inspect 原始日志只读导入（不执行日志内容）。

主断言 test_inspect_import_never_executes_log：日志里的命令/路径只是
数据；未知 schema 拒绝；同源重导幂等；aggregate 不制造样本。
"""
...
from motte_harness.inspect import InspectHarness, InspectLogError, import_inspect_log
from motte_harness.parsers.inspect import parse_inspect_log

SAMPLE_LOG = "\n".join([
    json.dumps({"model": "test-model", "task": "hello-task", "eval": {"scorer": "exact"}}),
    json.dumps({"sample": {
        "id": "sample-1", "epoch": 1,
        "scores": {"exact": {"name": "exact", "value": 1.0}},
    }}),
    json.dumps({"sample": {
        "id": "sample-2", "epoch": 1,
        "scores": {"exact": {"name": "exact", "value": 0.0}},
    }}),
    json.dumps({"event": "model call recorded"}),
])


class TestParser:
    def test_parses_header_and_samples_with_native_scores(self):
        parsed = parse_inspect_log(SAMPLE_LOG)
        assert parsed["schema"] == "inspect-eval-log-v1"
