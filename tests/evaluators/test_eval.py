...
def test_exact_regex():
    assert exact_match("a", "a").passed
    assert regex_match("abc", "b").passed


def test_aggregate_distinguishes_missing():
    assert aggregate([1, None, "unsupported"]) == {"pass": 1, "missing": 1, "unsupported": 1}
...
def test_gate_direction():
    assert RegressionGate(0.8, "gte").check(0.9)
