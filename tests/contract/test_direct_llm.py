...
from motte_contracts.direct_llm import (DATASET_ID, DATASET_VERSION, DEFAULT_SCORER, EVAL_KEY,
                                        SCORER_VERSION, SUITE, effective_scorers,
                                        import_direct_llm_jsonl, is_dataset, is_scenario,
                                        normalize_scorer, scenario_for, validate_dataset,
                                        validate_scenario)
...
def raw_source(rows=None):
    rows = rows if rows is not None else [
        {"input": f"SYNTHETIC question {i}", "expected": f"answer-{i}"} for i in range(6)]
    return ("\n".join(json.dumps(row, ensure_ascii=False) for row in rows) + "\n").encode()


def imported(raw=None, *, name="direct-llm-synthetic", scorer=None, version="1"):
    return import_direct_llm_jsonl(raw if raw is not None else raw_source(), name=name,
                                   version=version, license_id="synthetic-only", scorer=scorer,
...
def test_import_freezes_the_whole_file_and_provenance():
    ...
    assert record["provenance"] == {"source": "synthetic", "license": "synthetic-only",
                                    "source_sha256": hashlib.sha256(raw).hexdigest(),
                                    "source_line_count": 6, "synthetic": True}
    assert record["cases_sha256"] == digest(record["cases"])
    assert record[EVAL_KEY] == {"suite": SUITE, "id": DATASET_ID, "version": DATASET_VERSION,
                                "selected_count": 6, "selection": "all-rows-in-file-order",
                                "scorer": DEFAULT_SCORER, "prompt_version": "direct-llm-verbatim-v1",
    ...
    assert record["cases"][0] == {"case_id": "direct-llm-synthetic-0000",
                                  "input": "SYNTHETIC question 0", "expected": "answer-0",
...
def test_import_accepts_prompt_alias_custom_ids_and_scorer_overrides():
    ...
    record = imported(raw)
    assert [case["case_id"] for case in record["cases"]] == [
        "direct-llm-synthetic-0000", "custom-7", "direct-llm-synthetic-0002",
    ...
    assert effective_scorers(record) == {
        "direct-llm-synthetic-0000": "exact", "custom-7": "exact",
        "direct-llm-synthetic-0002": "contains", "direct-llm-synthetic-0003": "exact"}
...
@pytest.mark.parametrize("rows,message", [
    ([{"input": "a", "expected": "b"}, {"input": "a", "expected": "b"},
      {"input": "a", "expected": "b"}, {"input": "a", "expected": "b"},
      {"input": "a", "case_id": "direct-llm-synthetic-0000", "expected": "b"}], "duplicate case_id"),
...
def test_scenario_for_and_validate_scenario():
    ...
    for broken in ({**scenario, "mode": "replay"}, {**scenario, "dataset": "no-at-sign"},
                   {**scenario, "version": ""},
...
def test_managed_versions_are_immutable_while_generic_records_still_upsert(tmp_path, backend):
    store = (InMemoryResourceStore() if backend == "memory"
...
def test_suite_dispatch_recognizes_both_suites():
    ...
    gsm8k = import_official_jsonl(
        b'{"question": "q", "answer": "r\\n#### 1"}\n', name="gsm8k-test", version="1",
        revision="synthetic", license_id="synthetic-only", synthetic=True, scope="full")
    assert suites.suite_of(direct) == SUITE
    assert suites.suite_of(gsm8k) == "gsm8k"
    assert suites.suite_of({"name": "x", "version": "1"}) is None
    assert suites.is_managed({"name": "x", "version": "1", EVAL_KEY: {"suite": SUITE}}) is False
    suites.validate_dataset(direct)
    suites.validate_scenario(scenario_for(direct, version="1"))
...
def test_gsm8k_provenance_carries_an_explicit_suite_marker():
    ...
    dataset = import_official_jsonl(raw, name="gsm8k-test", version="1", revision="synthetic",
                                    license_id="synthetic-only", synthetic=True, scope="full")
...
def test_run_level_selection_is_shared_with_gsm8k():
    ...
    picked = select_cases(record, {"mode": "ids", "case_ids": ["direct-llm-synthetic-0003",
                                                              "direct-llm-synthetic-0001"]})
