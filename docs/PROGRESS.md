...
M6 完成（2026-09-21；分支 codex/m6-experiments-comparison-gates 已合并主干并推送：merge `aee31a9`，PG 夹具修复 `d0826c9`，本地与远程 SHA 一致；CI 上 M6 全部用例含真实 PostgreSQL 参数组通过，剩余 3 项 CI 失败经 node-id 比对为 M5 修复批次既有回归）：Experiment/Comparison/Statistics/Baseline/Gate 全链路实现并逐包验收——冻结协议与真值表；三级比较结论；版本化 metric registry 与 disposition 真值表；规则化 Gate 引擎（六类决策/冻结优先级/experimental fail-closed）；statistical_policy@1（Task 聚类 bootstrap/pass@k=0.7）；回归分类 8 类；ExperimentService 只编排既有 RunService/Dispatcher；Baseline v2 三存储+0014 迁移；API/CLI/Web 三端统一入口与 exporter v1（JSON/JUnit、退出码 0-6）。离线门禁全绿（与基线精确 node-id 差分仅 1 项迁移链登记，更新后通过）；CLI 实测退出码 0/1/2/5；浏览器实测实验/比较/基线（CAS 冲突）/门禁四页；DeepSeek V4.1 Flash 有界 live 验收（4 调用/316 tokens/4 对；重复 compare/gate/export 零新增调用）。已知边界：真实 PG 无环境（skipif 如实登记）、Judge 人工校准缺失（正式门禁 fail-closed）、GSM8K/DirectLlm 旧 compare 页本地自算为 M5 遗留债务。逐目标/逐验收证据见 [M6 验证记录](verification/M6.md)。

M7 实施完成（2026-09-22，分支 codex/m7-sdk-migration-release，详见 [M7 验证记录](verification/M7.md)）：T00 冻结 [sdk-and-migration 协议](protocols/sdk-and-migration.md)（SDK 错误/能力/幂等/重试、SSE gap+snapshot、CLI 双模式、pytest/Trace/exporter v1 扩展、导入契约、备份屏障/staging 恢复、GC tombstone、单用户安全、打包与发布状态机）。T01-T05：类型化同步 SDK（MotteClient，httpx，惰能力握手，POST 不盲重）+ 服务端 run 创建幂等（request_key→确定性 run_id+持久注册表）+ capabilities 端点 + SSE motte-gap/snapshot/500 上限；CLI local/server 显式分流（server 只走 HTTP，断连非零退出零本地副作用）+ run 生命周期子命令 + backup --consistent/restore --staging/restore-guard/maintenance/gc/import 命令接线；opt-in pytest 门禁插件（普通 pytest 零模型零 Run）+ @motte_trace 单次执行保证；13 包真实 build-system/extras + make wheels + checkout 外干净 venv 安装测试 + CI packaging job。T06-T08：ImportManifest/来源安全加载/dry-run 零修改/checkpoint apply/resume/受限 rollback（共享工件保护），imported Run 永不入队、缺字段保持 unknown、legacy summary 不伪造逐题分。T09-T10：维护屏障（API 503+Worker 拒领）+ 引用制工件快照 + Manifest v2 + staging 全链校验恢复 + 恢复守卫；GC 默认 dry-run+pin/引用保护+tombstone；Host/Origin/Bearer 中间件（A17/A18 测试）。T11-T12：release smoke（e2e/旧库升级/恢复演练/冻结退出码）、支持矩阵（tested/blocked/not_run 如实）、release notes、切换手册、能力映射、cutover readiness 门禁（三替代场景+文档完整性）。离线门禁：M7 专项命令块 514+15 passed（2 项 terminalbench 为基线同败 Windows 环境族）、全量 232 failed 与基线精确 node-id 差分为 0 新增、ruff/mypy/compileall/openapi 无漂移、web 286 passed+build 通过。边界（如实）：无 Docker/PG 环境（Compose build/真实 PG/pg_dump blocked，CI Linux+真实 PG 部分覆盖）、真实旧导出与切换需单独授权 → 阶段结论 offline_verified + external_pending，未声明 stable_supported/cutover_ready。

M7 规划启动（2026-09-22）：以 M6 主干合入点 `aee31a9` 为输入，新增 [M7 开工交接](superpowers/plans/2026-09-22-m7-kickoff.md)、[权威执行计划](superpowers/plans/2026-09-22-m7-execution.md)、[开发 Agent 提示词](prompts/M7-development-agent.md) 和 [验证入口](verification/M7.md)。M7 当前为 planned/not_run，先冻结 SDK、SSE、CLI、迁移、备份、安全、支持矩阵和 cutover 契约；不得把 M6 的未验证 PG/Docker/Harbor/Judge 边界宣传成 M7 stable。
