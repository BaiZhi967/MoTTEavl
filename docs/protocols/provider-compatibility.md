...
- **PriceTable**（`price_tables` 资源，`(model_id, version)` 版本化）：多少钱——Run 创建期解析：manifest 显式 `price_table_version` > 连接 legacy payload > 该模型最新版本（自然排序）。`input_per_million` / `output_per_million` 必须采用表内 `currency` 的单位；新表需显式填三位大写币种代码，旧表缺省为 `USD`。Provider 成本回执与 Worker 进度携带币种，报告/比较按币种分列，不作隐式汇率换算。旧运行缺币种的成本收据不能仅凭价格表版本名重解释或覆盖。
