...
| Terminal-Bench（Harbor）页面 | 操作页沿用 `.operate-grid` / `.operate-card`：任务筛选（多选清单）、Agent Profile 与重复次数、预算与限制、预检与创建四卡；预检未通过时创建按钮禁用并逐条列出原因码 + 服务端登记的可执行说明（`.failure-list`）。监控页 `.tb-tree` 呈 Job（Run）→ Task → Trial 三层嵌套清单，`.tb-node` 为单行节点（刷新用 `icon-btn`），queued 且无 Trial 用 `.empty` 显式空态。结果页 6 张指标卡（通过率 / 有效 Trial / 有效 Task / 无效 Trial / 已知成本 / 每成功成本），Trial 钻取用 `.drill-detail` + 通用表格，终端文本用 `.terminal-log`（`--bg-subtle` 底、1px 边框、mono 12px、内部滚动、`white-space: pre-wrap`）。未知身份 / 成本 / reward 一律显示「未知」，缺工件显示「缺工件」，绝不填 0；Trial 或 Task 切换时按 run+Trial+工件身份重挂载下钻视图，晚到响应不得覆盖新选择 |
