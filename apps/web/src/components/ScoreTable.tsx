        ...
        <tbody>
          {scores.map((score, index) => (
            <tr key={score.case_id ?? score.evaluator ?? index}>
              <td className="mono">{score.case_id ?? score.evaluator ?? "—"}</td>
              <td className={score.passed === true ? "pass" : score.passed === false ? "fail" : undefined}>
                {score.passed === true ? "通过" : score.passed === false ? "未通过" : "未判定"}
              </td>
            </tr>
          ))}
          {scores.length === 0 && (
