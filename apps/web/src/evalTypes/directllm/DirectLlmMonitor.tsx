    ...
    <div className="progress-grid" role="list" aria-label="逐题进度">
      {gridCells(run).map((cell) => (
        <span
          ...
          role="listitem"
          title={`${cell.caseId}：${cell.state === "pass" ? "通过" : cell.state === "fail" ? "不通过" : "未判定"}`}
        >
...
function DirectLlmRowDetail({ runId }: { runId: string }) {
  ...
  return (
    <div className="page">
      <BatchMonitor
        ...
        renderDetail={(runId) => <DirectLlmRowDetail runId={runId} />}
      />
    </div>
  );
}
