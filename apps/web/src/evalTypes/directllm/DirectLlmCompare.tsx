...
function collect(run: RunRecord, report: { cost?: { total?: number | null } } | null): Column {
  ...
  const judged = scores.filter((score) => score.judged ?? score.outcome !== "no_expectation");
...
export function DirectLlmCompare() {
  ...
  const datasetScorerName = columns[0]?.run.manifest?.benchmark_provenance?.scorer as string | undefined;
  /* 子集运行可以各选不同题目：判定期数不一致时通过率不可直接横向比较。 */
  const sizes = [...new Set(columns.map((column) => column.judged))];
    ...
    <div className="page">
      <section className="panel detail">
        ...
        <p className="hint">
          ...
          {datasetScorerName ? ` · 数据集默认评分器 ${datasetScorerName}` : ""}
        </p>
        {sizes.length > 1 && (
          <p className="hint">
            各列判定题数不同（{sizes.join(" / ")} 题），通过率不是同一题集上的比较。
          </p>
        )}
        <table>
          <thead>
            ...
            <li key={caseId}>
              ...
              {openCase === caseId && (
                <div className="drill-detail">
                  <p><span className="field-label">题面</span>{snapshotCases.find((c) => c.case_id === caseId)?.input ?? "（题面缺失）"}</p>
                  <p>
                    <span className="field-label">期望</span>
                    <span className="mono">{snapshotCases.find((c) => c.case_id === caseId)?.expected ?? "（无判定）"}</span>
                  </p>
                  <p>
                    <span className="field-label">评分器</span>
                    <span className="mono">
                      {scorerShort(columns[0]?.run.scores?.find((s) => s.case_id === caseId)?.scorer
                        ?? snapshotCases.find((c) => c.case_id === caseId)?.metadata?.scorer
                        ?? datasetScorerName)}
                    </span>
                  </p>
                  {columns.map((column) => {
